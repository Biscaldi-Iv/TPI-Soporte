...
def home():
    ...
    if session['user'] is not None:
        ...
        context = {'user': user_logic.get(session.get(
            'user')), 'top10': top10, 'mytop': mytop}
...
def questions():
    if request.method == 'POST':
        ...
        if nivel <= 10:
            ...
            p = PreguntasLogic()
            pregunta, correcta, incorrecta, imagen = p.getRandomQuestion(
                4)
            contexto = {"pregunta": pregunta, "correcta": correcta,
                        "incorrecta": incorrecta, "imagen": imagen}
